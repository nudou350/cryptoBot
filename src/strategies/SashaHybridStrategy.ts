 ...
 */
export class SashaHybridStrategy extends BaseStrategy {
  ...
  private analyzeEntry(
    ...
    if (this.marketRegime === 'ranging') {
      // Enter long at lower levels when oversold
      if (currentLevel <= 2 && rsi < 45) {
        ...
        return {
          ...
          stopLoss,
          takeProfit,
          reason: `Sasha-Hybrid: Ranging entry at level ${currentLevel} (RSI: ${rsi.toFixed(1)})`
        };
      }
    ...
    if (this.marketRegime === 'trending') {
      ...
      if (isUptrend && currentLevel <= 3 && priceVsEMA < 0.01 && rsi < 50) {
        this.currentPosition = 'long';
        this.entryPrice = currentPrice;
        this.entryLevel = currentLevel;
        this.positionScale = this.gridLevels[currentLevel].sizeMultiplier;
        ...
        return {
          ...
          stopLoss,
          takeProfit,
          reason: `Sasha-Hybrid: Trending entry at level ${currentLevel} (pullback)`
        };
      }
