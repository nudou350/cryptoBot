...
/**
 * Sasha Market Making Ladder Strategy
 *
 ...
 */
export class SashaMMLadderStrategy extends BaseStrategy {
  ...
  public analyze(candles: Candle[], currentPrice: number): TradeSignal {
    ...
    if (this.currentPosition === null) {
      // Only enter on strong trends with pullbacks
      const priceVsEMA = (currentPrice - currentEMA20) / currentEMA20;

      // Entry on pullback in uptrend
      if (isUptrend && priceVsEMA < -0.005 && trendStrength > 0.01) {
        ...
        return {
          ...
          stopLoss,
          takeProfit,
          reason: `Sasha-Ladder: Entry level 0 (pullback in uptrend, trend: ${(trendStrength * 100).toFixed(2)}%)`
        };
      }
    ...
    if (this.currentPosition === 'long') {
      ...
      if (this.currentLevel >= this.numberOfLevels - 1) {
        ...
        return {
          action: 'close',
          price: currentPrice,
          reason: `Sasha-Ladder: Reached top of ladder (profit: ${profitPercent.toFixed(2)}%)`
        };
      }
