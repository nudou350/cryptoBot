...
/**
 * Sasha Liquidity Providing Strategy
 *
 ...
 */
export class SashaLiqProvidingStrategy extends BaseStrategy {
  ...
  public analyze(candles: Candle[], currentPrice: number): TradeSignal {
    ...
    // BUY LOGIC: Price at lower half of grid
    if (currentLevel < midLevel) {
      ...
      return {
        ...
        stopLoss,
        takeProfit,
        reason: `Sasha-Liq: Buy at level ${currentLevel}/${this.numberOfLevels} (spread capture)`
      };
    }
    ...
    if (currentLevel >= midLevel) {
      ...
      if (this.lastTrade === 'buy') {
        ...
        return {
          action: 'close',
          price: currentPrice,
          reason: `Sasha-Liq: Close at level ${currentLevel}/${this.numberOfLevels} (taking spread profit)`
        };
      }
    }
